...
let espacio_chiste = document.getElementById("espacio_chiste");
//Icono borrar
...
boton_agregar.onclick = (e) => {
    ...
    //Notificación de tostify de ToDo agregado
    Toastify({
        text: `Agregado: ${descripcion}`,
        duration: 3000,
        style: {
...
const funcion_borrar = (e) => {
    ...
    Toastify({
        ...
        duration: 3000,
        style: {
            background: "linear-gradient(to right, #F88379, #811331)",
            fontFamily: "MontserratRegular"
        }
    }).showToast();
}
...
fetch('https://v2.jokeapi.dev/joke/Any?lang=es')
    ...
    .then((object) => { 
        console.log(object)
        let {type, delivery, setup, joke} = object;
        let titulo_chiste = "<h2>Empezá con una sonrisa :)</h2>"
        let pie_chiste = 'Powered by <a target="_blank" href="https://sv443.net/jokeapi/v2/">Jokeapi</a>'
        if(type === "twopart") {
            espacio_chiste.innerHTML = `${titulo_chiste}<div id="contenedor_chiste"><p>${setup}</p><p>${delivery}</p><p class="pie_chiste">${pie_chiste}</p></div>`
        } else {
            espacio_chiste.innerHTML = `${titulo_chiste}<div id="contenedor_chiste"><p>${joke}</p><p class="pie_chiste">${pie_chiste}</p></div>`
        }
     });